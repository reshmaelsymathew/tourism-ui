<template>
  <div class="min-h-screen bg-gray-100 p-8">
    ...
    <div class="chart-container mb-6" v-if="actionTypeCounts.length">
      <ActionTypeCountChart :actionTypeCounts="actionTypeCounts" />
    </div>
    ...
    <div class="chart-container-count" v-if="actionTypeEventDateCounts.length">
      <ActionTypeEventDateChart :actionTypeEventDateCounts="actionTypeEventDateCounts" />
    </div>
  </div>
</template>

<script>
import ActionTypeCountChart from './ActionTypeCountChart.vue';
import ActionTypeEventDateChart from './ActionTypeEventDateChart.vue';
import axios from 'axios';

export default {
  components: {
    ActionTypeCountChart,
    ActionTypeEventDateChart,
  },
  data() {
    return {
      actionTypeCounts: [],
  ...
  methods: {
    async fetchData() {
      try {
        const actionTypeCountResponse = await axios.get('http://localhost:8080/api/eventlogap/countByActionType');
        ...
        const actionTypeEventDateResponse = await axios.get('http://localhost:8080/api/eventlogap/countByActionTypeAndEventDate');
